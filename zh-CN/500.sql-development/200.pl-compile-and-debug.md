...
本文档以创建 OceanBase Oracle 类型的存储过程为例，在数据库 GshH 中创建存储过程 PROC_VARCHAR2。
...
2. 在 SQL 开发窗口的左侧导航栏 > 存储过程列表中，选择 **<UI-TERM key="odc.EditorToolBar.actions.pl.Compile">编译</UI-TERM>** 存储过程 PROC_VARCHAR2。
...
2. 设置参数值后，单击 **<UI-TERM key="odc.Datasource.NewDatasourceDrawer.Ok">确定</UI-TERM>**。

    <main id="notice" type="notice">
      <h4>注意</h4>
      <p>OceanBase V4.0.0 及之后的版本，调试前需要获取调试权限。示例：<code> GRANT DEBUG CONNECT SESSION TO GSH; GRANT DEBUG ANY PROCEDURE TO GSH;</code></p>
    </main>
