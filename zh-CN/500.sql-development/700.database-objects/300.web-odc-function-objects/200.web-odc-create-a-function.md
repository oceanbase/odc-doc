...
1. 参数即调用函数时传入的信息，需指定参数信息：
    ...
    * Oracle 模式：需指定参数的 **<UI-TERM key="workspace.window.sql.explain.tab.summary.columns.name">名称</UI-TERM>**、**<UI-TERM key="odc.component.ProcedureParam.Mode">模式</UI-TERM>**、**<UI-TERM key="odc.component.TaskTable.Type">类型</UI-TERM>** 和 **<UI-TERM key="odc.component.ProcedureParam.DefaultValue">默认值</UI-TERM>** **。**

    * MySQL 模式：需指定参数的 **<UI-TERM key="workspace.window.createFunction.paramName">名称</UI-TERM>**、**<UI-TERM key="odc.components.PLBatchCompilePage.Type">类型</UI-TERM>** 和 **<UI-TERM key="odc.components.ProcedurePage.Length">长度</UI-TERM>** 。
