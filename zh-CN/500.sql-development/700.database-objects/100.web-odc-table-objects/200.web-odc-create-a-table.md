...
<main id="notice" type='explain'>
   <h4>说明</h4>
   <p>在左侧导航栏中，右键鼠标单击表列表中的表名，通过弹出的管理操作列表（包括 <strong>查看表结构</strong>、<strong>查看表数据</strong>、 <strong>导入</strong>、 <strong>导出</strong>、 <strong>下载</strong>、 <strong>模拟数据</strong>、<strong>复制</strong>、<strong>删除</strong> 和 <strong>刷新</strong>），可快速管理和操作目标对象。</p>
</main>
