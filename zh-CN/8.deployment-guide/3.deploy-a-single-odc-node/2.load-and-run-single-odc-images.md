...
* X86 镜像：[\[下载\]](https://ob-front.oss-cn-hangzhou.aliyuncs.com/client/4.0.0/obodc4.0.0.tar.gz)<br>
* ARM 镜像：[\[下载\]](https://ob-front.oss-cn-hangzhou.aliyuncs.com/client/4.0.0/obodc4.0.0_arm.tar.gz)
...
| ODC_APP_EXTRA_ARGS    | 其它需要指定的 App 参数。<br> 例如 `--server.servlet.session.timeout=10m`，表示 Session 过期时间为 10 分钟。<br> 如需启用 AI 敏感列识别功能，可配置：`--odc.ai.enabled=true --odc.ai.api-key=<Your Api Key> --odc.ai.base-url=<Your Base Url> --odc.ai.model=<Your Model Name>`。<br> 推荐使用魔搭社区模型 fylfy111/SensitiveColumn（基于 qwen2.5 7B Instruct），建议 --max-model-len=4096，至少需要单卡 3090 服务器部署。默认值：空。                         |
