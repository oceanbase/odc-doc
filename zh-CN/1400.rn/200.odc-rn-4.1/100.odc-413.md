...
* 数据库变更
  ...
  * 创建数据库变更时，设置 **<UI-TERM key="odc.components.CreateAsyncTaskModal.ExecutionTimeout">执行超时时间</UI-TERM>** 无效。

* 连接管理

  * 创建连接时，输入数据库用户名和数据库密码后单击 **<UI-TERM key="odc.component.SysFormItem.TestConnection">测试连接</UI-TERM>** 无法识别云服务的连接地址。
