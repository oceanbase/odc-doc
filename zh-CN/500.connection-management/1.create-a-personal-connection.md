...
ODC 支持与 OceanBase 数据库建立连接，连接成功后，用户可以通过 ODC 对所连接的数据源（数据库对象和资源）进行编辑和管理。ODC 支持连接专有云和公有云环境中的 OceanBase MySQL 模式和 Oracle 模式数据库。
...
1. 登录 Web 版 ODC 后，在项目协同窗口的左侧导航栏中，单击 **数据源** > **新建数据源**。
   ...
   ![1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/420/quickstart/webodc/connect/web%20connection1.png)
...
2. 在弹出的 **新建数据源** 面板中填写或选择以下信息。
   ...
   ![2](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/420/quickstart/webodc/connect/2.png)
...
4. 创建完成后，可在数据源列表中查看创建的数据源。
   ...
   ![4](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/420/quickstart/webodc/connect/web%20connection4.png)
...
2. 在弹出的 **批量导入** 面板中单击 **下载模板**，填写需要导入的数据源信息：
   ...
   <main id="notice" type='explain'>
   <h4>说明</h4>
   <p>批量导入数据源的模板文件中，无需填写资源组名称、连接状态、只读账号用户名和只读账号密码。</p>
   </main> 
