...
| **一级参数名**              | **二级参数名**                                                | **三级参数名**                   | **四级参数名** | **<UI-TERM key="odc.SSO.SSODetailDrawer.Type">类型</UI-TERM>**                | **<UI-TERM key="odc.component.ProcedureParam.DefaultValue">默认值</UI-TERM>**  | **说明**  |
|------------------------|----------------------------------------------------------|-----------------------------|-----------|-----------------------|----------|----------------------------------------------|
...
| **<UI-TERM key="odc.component.EditPLParamsModal.Parameter">参数名</UI-TERM>** | **<UI-TERM key="odc.ResourceTree.Nodes.type.Type">类型</UI-TERM>** | **生效范围** | **说明** |
| ------ | ------ | ------ | ------ |
...
| database.owner.ids | String | 审批集成 | 数据库 owner 的用户 ID 列表，JSON 数组格式。 |
| database.owner.accounts | String | 审批集成 | 数据库 owner 的用户账号名列表，JSON 数组格式。 |
| database.owner.names | String | 审批集成 | 数据库 owner 的用户姓名列表，JSON 数组格式。 |
| database.name | String | 审批集成 | 数据库名。 |
| environment.name | String | 审批集成 | 环境名。 |
| task.description | String | 审批集成 | 工单描述。 |
| risk.level | String | 审批集成 | 风险等级。 |
| project.owner.ids | String | 审批集成 | 项目 owner 的用户 ID 列表，JSON 数组格式。 |
| project.owner.accounts | String | 审批集成 | 项目 owner 的用户账号名列表，JSON 数组格式。 |
| project.owner.names | String | 审批集成 | 项目 owner 的用户姓名列表，JSON 数组格式。 |
| odc.task.url | String | 审批集成 | 该工单的访问地址。 |
...
1. 在项目协同窗口，单击 **<UI-TERM key="odc.component.AuthNode.ExternalIntegration">外部集成</UI-TERM>** > **<UI-TERM key="odc.ExternalIntegration.SqlInterceptor.ApprovalIntegration">审批集成</UI-TERM>** > **新建审批集成**。
...
3. 单击 **<UI-TERM key="odc.components.CreateSQLPlanTaskModal.Save">保存</UI-TERM>**，完成新建审批集成。
...
在审批集成列表中，单击操作项下的 **<UI-TERM key="odc.Secure.MaskingAlgorithm.View">查看</UI-TERM>**/**<UI-TERM key="odc.Env.components.EditRuleDrawer.Edit">编辑</UI-TERM>**/**<UI-TERM key="src.component.Task.StructureComparisonTask.CreateModal.4DAACA54">删除</UI-TERM>** 按钮，可以查看/编辑/删除审批集成信息。
