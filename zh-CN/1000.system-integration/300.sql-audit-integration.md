...
| **一级参数名** | **二级参数名**   | **三级参数名**                | **四级参数名** | **<UI-TERM key="odc.ResourceTree.Nodes.type.Type">类型</UI-TERM>**                | **<UI-TERM key="workspace.window.createFunction.defaultValue">默认值</UI-TERM>**  | **说明**  |
|-----------|---------------|-----------|-----------|-------------|----------|--------------|
| http      | connectTimeoutSeconds                                        | -                        | -         | Integer               | 5        | HTTP 请求的连接超时时间，单位：秒                          |
| http      | socketTimeoutSeconds                                         | -                        | -         | Integer               | 30       | HTTP 请求的 Socket 超时时间，单位：秒                    |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | method                   | -         | Enum                  | POST     | HTTP 请求方法，支持的枚举值：GET \| POST \| PUT \| PATCH |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.src.component.Task.AsyncTask.CreateModal.SQLCheck">SQL 检查</UI-TERM>** 请求） | url                      | -         | String                | \<NULL\> | HTTP 请求地址                                    |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.src.component.Task.AsyncTask.CreateModal.SQLCheck">SQL 检查</UI-TERM>** 请求） | headers                  | -         | Map\<String, String\> | \<NULL\> | HTTP 请求头                                     |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.src.component.Task.AsyncTask.CreateModal.SQLCheck">SQL 检查</UI-TERM>** 请求） | queryParameters          | -         | Map\<String, String\> | \<NULL\> | HTTP 请求参数                                    |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | body                     | type      | ENUM                  | \<NULL\> | HTTP 请求体的格式，支持的枚举值：FORM_DATA                 |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | body                     | content   | Object                | \<NULL\> | HTTP 请求体的内容                                  |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | requestEncrypted         | -         | Boolean               | false    | HTTP 请求体是否加密                                 |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | requestSuccessExpression | -         | String                | \<NULL\> | 根据响应体解析判断请求是否执行成功的表达式，采用 SPEL 语法规范           |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.src.component.Task.AsyncTask.CreateModal.SQLCheck">SQL 检查</UI-TERM>** 请求） | inWhiteListExpression    | -         | String                | \<NULL\> | 根据响应体解析判断待执行 SQL 是否在白名单中的表达式，采用 SPEL 语法规范    |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | inBlackListExpression    | -         | String                | \<NULL\> | 根据响应体解析判断待执行 SQL 是否在黑名单中的表达式，采用 SPEL 语法规范    |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.EditorToolBar.actions.sql.SqlCheck">SQL 检查</UI-TERM>** 请求） | needReviewExpression     | -         | String                | \<NULL\> | 根据响应体解析判断待执行 SQL 是否需要审批的表达式，采用 SPEL 语法规范     |
| api       | check（用于向 **外部 SQL 审核系统** 发起 **<UI-TERM key="odc.src.component.Task.AsyncTask.CreateModal.SQLCheck">SQL 检查</UI-TERM>** 请求） | responseEncrypted        | -         | Boolean               | false    | HTTP 响应体是否加密                                 |
...
| **<UI-TERM key="odc.components.PLDebugResultSet.Parameter.1">参数名</UI-TERM>** | **<UI-TERM key="odc.src.constant.label.Type">类型</UI-TERM>** | **生效范围** | **说明** |
| ------ | ------ | ------ | ------ |
...
1. 在项目协同窗口，单击 **<UI-TERM key="odc.page.ExternalIntegration.ExternalIntegration">外部集成</UI-TERM>** > **<UI-TERM key="odc.ExternalIntegration.SqlInterceptor.SqlAuditIntegration">SQL 审核集成</UI-TERM>** > **新建 SQL 审核集成**。
...
3. 单击 **<UI-TERM key="src.component.ODCSetting.AB9A3FA4">保存</UI-TERM>**，完成新建 SQL 审核集成。
...
在 SQL 审核集成列表中，单击操作项下的 **<UI-TERM key="odc.ExternalIntegration.SqlInterceptor.View">查看</UI-TERM>**/**<UI-TERM key="odc.ResourceTree.actions.Editing">编辑</UI-TERM>**/**<UI-TERM key="src.page.Project.Notification.components.B65E55FA">删除</UI-TERM>** 按钮，可以查看/编辑/删除 SQL 审核集成信息。
