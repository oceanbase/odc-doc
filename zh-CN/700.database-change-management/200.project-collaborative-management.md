...
拥有表权限的用户可以在 SQL 开发窗口中创建和管理表，具体操作请参见 [SQL 语句创建和管理表](../500.sql-development/100.sql-editing-and-execution.md) 和 [可视化方式创建和管理表](../500.sql-development/700.database-objects/100.web-odc-table-objects/200.web-odc-create-a-table.md)。

### 方法二：申请表权限

ODC 用户可以通过 **申请表权限** 以获取查询/导出/变更表权限。
...
2. 选择项目、表、表权限类型、权限有效期并输入申请原因后，单击 **新建**，创建表权限申请。
