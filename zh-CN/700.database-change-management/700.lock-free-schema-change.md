...
变更数据库表结构时会阻止其他用户访问该表，因此需要锁表。无锁结构变更的原理是创建临时表，将原表数据拷贝到临时表中，将临时表结构变更，再全量拷贝数据到原表，最后切换新旧表。
...
- 如果 OceanBase MySQL 模式版本小于 4.3.0，ODC 通过锁定用户且关闭会话的方式避免表名切换过程中数据的写入。
...
9. 锁定账号/锁表。
   ...
   - 对于 OceanBasese MySQL V4.2.5 及之后的版本，变更数据库表结构时不再通过锁定账号进行锁表，您需要确认 OBProxy 版本为 V4.3.1 及之后，并在无锁结构变更任务前执行以下操作进行锁表，否则锁表失败。
      ...
      3. 验证锁表是否成功，通过 ODC 执行 <code>lock table</code> 命令锁定一个临时表后，使用新链接尝试插入数据到临时表中，插入数据被阻塞则表示锁表任务生效。
...
10. 新建无锁结构变更任务。
    ...
    1. 在 **工单** 页签中单击 **无锁结构变更** > **新建无锁结构变更**。
       ...
       <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/420/sql-development/10.table%20structure%20change/4.png" width="900">

    2. 在 **新建无锁结构变更** 页面中，输入以下信息。

       <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/423/700.database-change-management/700.table-structure-change/5.png" width="900">
    ...
    4. 任务生成后可以在 **工单** > **无锁结构变更** 列表中查看任务信息。
       ...
       <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/423/700.database-change-management/700.table-structure-change/6.png" width="900">
...
<img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/423/700.database-change-management/700.table-structure-change/8.png" width="900">

在任务详情面板中，单击 **任务流程** 页签查看发起任务状态、审批状态、执行状态和执行结果等信息。

### 查看执行记录 

<img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/423/700.database-change-management/700.table-structure-change/9.png" width="900">

在任务详情面板中，单击 **执行记录** 页签查看新表和源表 DDL。

### 查看任务日志

<img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/423/700.database-change-management/700.table-structure-change/10.png" width="900">
