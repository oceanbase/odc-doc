...
2. [安装 ConfigServer](https://github.com/oceanbase/oceanbase/tree/master/tools/ob-configserver)。ConfigServer 为 OceanBase 和 OBProxy 存取 OceanBase rs_list 的接口, OceanBase 配置 configserver 地址完成后，OceanBase 会自动注册 rs_list 到 config server。
...
3. [安装企业版 OMS](https://www.oceanbase.com/docs/community-oms-cn-1000000000248267)。本文以企业版 OMS 为例，您可以通过 Docker 获取，您也可以按需选择社区版 OMS 并完成配置。
...
5. 配置 ODC 元数据信息，启用无锁结构变更。
   ...
   a. ODC 配置 OMS 信息。
      ...
      - 示例：
         ...
         ```shell
         #配置 oms_url，填写 oms 的访问地址：http://localhost:8088
