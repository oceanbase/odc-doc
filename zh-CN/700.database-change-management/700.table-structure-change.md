...
示例：OceanBase 社区环境下，将表 student 中的列 birth 类型修改为 datetime。
...
3. [安装 OMS-CE 4.2.0_ce](https://www.oceanbase.com/docs/community-oms-cn-1000000000248267)。
   ...
   可通过 Docker 获取支持无锁结构变更的 OMS 版本。

   ```shell
   sudo docker  reg.docker.alibaba-inc.com/oceanbase/oms:feature_4.2.0_ce
   ```
...
5. 配置 ODC 元数据信息，启用无锁结构变更。
   ...
   a. ODC 配置 OMS 信息。
      ...
      连接 ODC 的元数据库， 配置 OMS 的 url 和登陆授权信息 oms_url 构成为 ${oms_host}: ${oms_port} oms_authornazition 构成为 username:password 的 Base64 编码。
    ...
    b. 打开无锁结构变更。
       ...
       由于无锁结构变更目前在阿里云数据库 OceanBase 已开放，而 OceanBase 社区版本默认未开放，需要配置 ODC 元数据库，通过 sql 打开配置。
...
7. [创建项目、项目中添加数据库、添加数据库](../300.quickstart/200.web-odc-quickstart/300.quickstart-using-web-odc.md)。
