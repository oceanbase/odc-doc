...
- 以下情况不支持清理：
  ...
  - OceanBase MySQL 及 MySQL 数据源若表中包含 bit、enum、set、xml 、geometry字段类型，不支持进行清理。
