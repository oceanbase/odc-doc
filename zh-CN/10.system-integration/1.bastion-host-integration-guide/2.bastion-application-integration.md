...
ODC 地址为 `http://192.xxx.x.x:2883`，参数为 `abc`, 对应跳转地址为 `http://192.xxx.x.x:2883/#/gateway/abc`。

调用示例代码：

```unknow
// Web 页面内 JavaScript 调用跳转到 ODC
window.open('192.xxx.x.x:2883/#/gateway/' + param)

// chrome 唤起
chrome.exe --new-window 192.xxx.x.x:2883/#/gateway/param
```
