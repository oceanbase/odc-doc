...
* X86 镜像：[\[下载\]](https://ob-front.oss-cn-hangzhou.aliyuncs.com/client/4.0.0-patch/obodc4.0.0-bp.tar.gz)<br>
* ARM 镜像：[\[下载\]](https://ob-front.oss-cn-hangzhou.aliyuncs.com/client/4.0.0-patch/obodc4.0.0-bp_arm.tar.gz)
...
```javascript
#!/usr/bin/env bash
docker run --volume   /var/log/odc:/opt/odc/log -v /var/data/odc:/opt/odc/data\
...
|-v                                        | 宿主机的 `/var/data/odc` 目录挂载到 ODC 容器内的 `/opt/odc/data` 目录。`/var/data/odc` 为宿主机的目录，可自定义，如不存在需先创建，创建目录的参考命令 `mkdir -p /var/data/odc`。|
