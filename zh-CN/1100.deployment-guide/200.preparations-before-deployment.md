...
2. 在租户下创建数据库和用户以作为 ODC 元数据库。
   ...
   <main id="notice" type='notice'>
      <h4>注意</h4>
      <ul>
      <li>用户名、密码和元数据库名称无限制。本章节中的示例数据库命名为 odc_metadb，数据库用户名和密码均为 obodc。</li>
      <li>元数据库需为 <strong>V1.4.79 及之后版本</strong>。
      <li>元数据库的字符集需为 <strong>utf8mb4</strong>，否则在填写字段时，输入内容会受到限制，例如不能使用 Emoji。</li>
      </ul>
   </main>
