...
5. 进入指定导出文件窗口，单击窗口中的 **浏览** 按钮，在弹出的文件资源管理器窗口中选择导出路径并在 **<UI-TERM key="odc.src.component.Task.ResultSetExportTask.DetailContent.FileName">文件名</UI-TERM>** 文本框中指定文件名称，在文件资源管理器窗口中单击右下角的 **<UI-TERM key="odc.components.CreateSQLPlanTaskModal.Save">保存</UI-TERM>** 按钮回到导出文件窗口，单击 **下一步** 。
...
9. 进入证书存储窗口，选择 **将所有证书都放入下列存储** ，并单击选项下的 **浏览** 按钮，在弹出的存储列表窗口中，选择 **受信任的颁发机构** ，然后单击窗口中的 **<UI-TERM key="src.component.Task.MutipleAsyncTask.components.Template.AB6525D4">确定</UI-TERM>** 按钮。回到证书存储窗口，单击 **下一步** 。
