...
1. 在 SQL 开发窗口，单击 **<UI-TERM key="odc.Workspace.ActivityBar.type.OMManagement">运维管理</UI-TERM>** > **<UI-TERM key="odc.SideBar.Manager.RecycleBin">回收站</UI-TERM>**。
...
2. 选择数据源 mysql420，并单击 **<UI-TERM key="src.page.Project.User.ManageModal.1C087F21">确定</UI-TERM>**。
...
3. 在 **<UI-TERM key="odc.SideBar.Manager.RecycleBin">回收站</UI-TERM>** 页面中，可以查询数据源 mysql420 下所有被删除的数据库对象。
...
1. 在回收站列表中，勾选表对象 ccc 后，单击 **<UI-TERM key="workspace.window.recyclebin.button.restore">还原</UI-TERM>**。
...
2. 在还原数据库对象页面中，可以重命名表对象名称，单击 **<UI-TERM key="workspace.window.recyclebin.button.restore">还原</UI-TERM>**。
...
3. 在 **<UI-TERM key="odc.component.SQLExecuteModal.SqlConfirmation">SQL 确认</UI-TERM>** 弹窗中，确认 SQL 语句后，单击 **<UI-TERM key="odc.TaskManagePage.component.TaskTools.Run">执行</UI-TERM>**。
