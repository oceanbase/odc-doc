...
6. 创建规则。
   ...
   - `parti_key`：该分区键类型为 `INT`，不属于时间类型，只能使用 **自定义** 作为分区创建策略。假定需要以当前时间为基准，间隔 1 个月生成若干分区。参见 **自定义分区创建策略** 一节中关于 **自定义分区创建策略** 的表述，该表达式可以按照如下步骤生成：
...
6. 创建规则。
   ...
   - `KEY`：该分区键类型为 `NUMBER`，不属于时间类型，只能使用 **自定义** 作为分区创建策略。假定需要以当前系统时间为基准，间隔 1 个月生成若干分区。参见 **自定义分区创建策略** 一节中关于 **自定义分区创建策略** 的表述，该表达式可以按照如下步骤生成：
