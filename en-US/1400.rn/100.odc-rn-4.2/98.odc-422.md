...
* The following table lists the features supported for MySQL data sources.
   ...
   | ------ | ------ | ------ |
   | Data lifecycle management | Data cleanup | Yes |
   |  | Data archiving (from a MySQL data source to an OceanBase MySQL data source, between MySQL data sources, and from an OceanBase MySQL data source to a MySQL data source) | Yes |
   | Change risk management | SQL development specifications | Yes |
   |  | Approval process configuration | Yes |
   | Security compliance | Operation records | Yes |
   |  | Data desensitization | Yes |
   | SQL window | SQL execution | Yes |
   |  | SQL check | Yes |
   |  | Auto-completion | Yes |
   |  | Execution plans | Yes |
   |  | Formatting | Yes |
   |  | Result set viewing | Yes |
   |  | Result set search | Yes |
   |  | Result set editing | Yes |
   |  | Result set export | No |
   |  | Script management | Yes |
   |  | Snippets | Yes |
   |  | Settings | Yes |
   | GUI-based object management | Tables | Yes |
   |  | Views | Yes |
   |  | Functions | Yes |
   |  | Stored procedures | Yes |
   | PL capabilities | Compilation | No |
   |  | Running | Yes |
   |  | Debugging | No |
   | Tickets | Import | No |
   |  | Export | No |
   ...
   |  | Synchronization of shadow tables | No |
   |  | SQL plans | Yes |
   |  | Partitioning plans | No |
   | Database O&M | Session management | Yes |
   |  | Session variable management | Yes |
   |  | Recycle bin management | No |
   |  | Command-line window | Yes |
...
* Data archiving
   ...
   * Fixed the issue where the **<UI-TERM key="odc.TaskManagePage.component.ApprovalModal.Pass">Approve</UI-TERM>** button is absent on the approval page when you request to abort a task.

   * Fixed the issue where the **<UI-TERM key="odc.src.page.Secure.RiskLevel.components.Cancel.1">Cancel</UI-TERM>** button is absent on the page of a scheduled data archiving task.
