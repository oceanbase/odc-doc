...
* Data sources
   ...
   * Fixed the issue where the value in the **<UI-TERM key="workspace.window.sql.explain.tab.detail.card.time.label.execTime">Execution Time</UI-TERM>** column is `0` for a session in MySQL mode of OceanBase Database or in a native MySQL database.
...
* PL objects
   ...
   * Fixed the issue where the `sys_refcursor` option is unavailable in the **<UI-TERM key="odc.components.ShowFunctionBaseInfoForm.ResponseType">Return Type</UI-TERM>** drop-down list.
...
* Projects
   ...
   * Fixed the issue where a user granted the personal workspace permission must log in again to use the personal workspace.
...
* Operation records
   ...
   * Fixed the issue where the value of the **<UI-TERM key="odc.Project.Database.DataSource">Data Source</UI-TERM>** column in the operation records is empty.
