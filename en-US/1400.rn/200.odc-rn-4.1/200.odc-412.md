...
OceanBase Developer Center (ODC) V4.1.2 released in March 2023 allows you to create multiple private connections, public connections, and users at a time, shows an information collection list on Client ODC, and optimizes features such as interactive operations in an SQL window, SQL execution, PL objects, result set editing, display of table data and view data, and shadow tables.
...
* SQL window
   ...
   * Before you execute an SQL statement, you can specify whether to enable **<UI-TERM key="odc.component.SQLConfig.ObtainTheColumnInformationOf">Obtain Column Information of Result Set</UI-TERM>**. If the table contains a large number of columns, we recommend that you do not enable this option. This way, column comments and editable columns are not obtained to reduce the SQL execution time.
...
* Third-party integration
   ...
   * You can use your account and password to log in to web ODC from the logon page of web ODC or a third party.

   * Single sign-on (SSO) supports the OpenID Connect (OIDC) protocol.

* Other features

   * Client ODC provides an information collection list to collect the usage frequency of some ODC features.
...
* Tools
   ...
   * In the command line tool of Client ODC on Windows, Chinese characters in commands may be garbled.
