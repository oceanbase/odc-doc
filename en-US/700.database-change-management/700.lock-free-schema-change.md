...
The principle of a lock-free schema change involves creating a temporary table, copying the data from the original table to the temporary table, altering the structure of the temporary table, then copying all the data back to the original table, and finally switching to the new table.
...
9. Lock an account / lock a table.

   - For versions of OceanBase MySQL prior to V4.2.5, table locking is not supported when you change the schema of a table. You need to specify the account to be locked. When switching to the new table (`Rename`), ODC will execute the `alter table user lock account` command for the locked account and close the session to terminate all processes under the locked account.

      <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odc/432/700.database-change-management/700.lock-free-schema-change/3EN.png" width="400">

   - For versions of OceanBase MySQL V4.2.5 and later, ODC no longer uses account locking for table locking when you change the schema of a table. You need to ensure that the OBProxy version is V4.3.1 or later, and perform the following steps before executing a lock-free schema change task to successfully lock the table; otherwise, the table lock will fail.

      1. Modify OBProxy parameters.
      ...
      3. To verify if the table lock is successful, execute the <code>lock table</code> command on a temporary table through ODC. Then, using a new connection to insert data into the temporary table. If the data insertion is blocked, it indicates that the table lock task is effective.
