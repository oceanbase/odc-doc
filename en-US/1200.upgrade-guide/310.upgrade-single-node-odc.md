...
1. Run the `docker ps -a` command in the command-line tool of the host to view the running containers. You can find the container ID of the earlier image in the `CONTAINER ID` column based on the image name in the `IMAGE` column. The result is as follows:
...
2. After you obtain the container ID, run the `docker stop <CONTAINER ID>;` command in the command-line tool to stop the ODC container of the earlier version. The result is as follows:
...
3. Run the `docker rm <CONTAINER ID>;` command in the command-line tool to delete the ODC image of the earlier version and release the port occupied. The result is as follows:
...
* Image for x86: [\[Download\]](https://obodc-front.oss-cn-beijing.aliyuncs.com/ODC%204.2.1/obodc4.2.1.tar.gz)<br>

* Image for Arm: [\[Download\]](https://obodc-front.oss-cn-beijing.aliyuncs.com/ODC%204.2.1/obodc4.2.1_arm.tar.gz)
...
| ODC_ADMIN_INITIAL_PASSWORD | The initial password of the administrator account of ODC. <main id="notice" type='notice'><h4>Notice</h4><p>The password must contain at least two digits, two uppercase letters, two lowercase letters, and two special characters. It must be 8 to 32 characters in length and cannot contain spaces. Supported special characters are . _ + @ # $ % </p></main> |
...
| ODC_APP_EXTRA_ARGS | Other app parameters. <br>For example, `--server.servlet.session.timeout=10m` indicates that the session timeout period is 10 minutes. By default, no parameter is specified.  |